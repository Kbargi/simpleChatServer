...
ThreadPool::ThreadPool(size_t threads_limit) : m_size(threads_limit < 2 ? 2 : threads_limit) {
...
void ThreadPool::stop() {
   ...
   for(std::vector<std::shared_ptr<std::thread>>::iterator it = m_pool.begin() ; it != m_pool.end() ; ++it) {
         if((*it)->joinable()) {
            (*it)->join();
         }
      }
