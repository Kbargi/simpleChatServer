...
void Config::parse() throw() {
   ...
   try {
      if(!file.is_open())
         throw std::runtime_error(std::string("Cannot open config file: " + m_mainConf));
	  ...
	  while ( getline (file, line) ) {
		 ...
		 if(line[0] == '[') {
			found = line.find(']');
            if(found == std::string::npos || found + 1 != line.size() || line.size() < 3) {
               throw std::runtime_error(
                                        std::string("Syntax error in configuration "
                                        "\nline: ") + line
                                        + std::string("\nfile: ") + m_mainConf
                                        + std::string("\nline number: ") + boost::lexical_cast<std::string>(lineNr)
      ...
      if(m_sections.empty()) {
          throw std::runtime_error(std::string("Configuration file is probably empty"));
      }
   }
      ...
      file.close();
      throw std::runtime_error(std::string("Unknown exception while parsing configuration file"));
   }
   file.close();
}
void Config::dump() {
